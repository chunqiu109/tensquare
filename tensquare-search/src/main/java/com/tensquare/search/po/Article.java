...
public class Article implements Serializable {
    ...
    @Field(//对应数据库中的列
            //类型
            type = FieldType.text,
            //是否生成索引,默认是true，如果是false，则保存的时候，不生成索引词条，看是否能被搜索
            index=true,
            //存储文档的时候，生成索引词条所使用的分词器，是否存储：就是是否在页面上显示
            analyzer="ik_max_word",
            //进行字段分词搜索的时候，使用的分词器，是否分词：就是便是搜索的时候是按照整体还是单个词汇匹配
